...
df.select(expr("DEST_COUNTRY_NAME as destination").alias("DEST_COUNTRY_nm"))\
.show(2)
...
randomDfs[0].show(100, False)   #.count() 
randomDfs[1].count() 
randomDfs[2].count() 
...
df3 = df.repartition(5, col("DEST_COUNTRY_NAME")).coalesce(2)
